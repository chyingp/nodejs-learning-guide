...
关于这个模块的重要性，基本不用强调了，在网络安全问题日益严峻的今天，网站采用HTTPS是个必然的趋势。

在nodejs中，提供了 https 这个模块来完成 HTTPS 相关功能，从官方文档来看，跟 http 模块非常相似。

同样的，下面也分别通过客户端、服务端的例子进行讲解。篇幅所限，本文不会对 HTTPS 协议本身做过多讲解，有问题欢迎留言交流。
...
要对外提供HTTPS服务，那么，我们需要有HTTPS证书。如果你已经有了HTTPS证书，那么可以跳过证书生成的环节。如果没有，可以参考如下步骤
...
接着，生成证书签名请求（csr是 Certificate Signing Request的意思）。
...
这里以我们最喜爱的12306最为例子。当我们访问12306的购票页面 https://kyfw.12306.cn/otn/regist/init 时，chrome会阻止我们访问，这是因为，12306的证书是自己颁发的，chrome无法确认他的安全性。
...
在浏览器里访问会有问题，同样的，通过 node创建的https客户端 发起请求，也会遇到同样问题。
